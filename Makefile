...
$(RBAC_OPENAPI_DOC) $(ENTERPRISE_OPENAPI_DOC) $(ENGINE_OPENAPI_DOC): $(PROJECT_ROOT) ## pull the enterprise external API swagger document
	...
	cp $(CLONE_DIR)/nextlinux_enterprise/swagger/enterprise_api_swagger.yaml $(ENTERPRISE_OPENAPI_DOC)
	cp $(CLONE_DIR)/nextlinux_enterprise/swagger/rbac_manager_swagger.yaml $(RBAC_OPENAPI_DOC)
	cp $(CLONE_DIR)/nextlinux_enterprise/services/api/swagger/swagger.yaml $(ENGINE_OPENAPI_DOC)
